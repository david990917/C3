...
#include <regex>
namespace btrblocks {
Relation::Relation() { columns.reserve(100); }
...
void Relation::addColumn(const string& column_file_path) {
	...
	if (std::regex_search(column_file_path, match, re) && match.size() > 1) {
		...
		bitmap_file_path        = bitmap_file_path.replace(
            bitmap_file_path.end() - column_type_str.length(), bitmap_file_path.end(), "bitmap");
...
vector<Range> Relation::getRanges(btrblocks::SplitStrategy strategy, u32 max_chunk_count) const {
	...
	if (max_chunk_count) { ranges.resize(std::min(static_cast<SIZE>(max_chunk_count), ranges.size())); }
...
Chunk Relation::getChunk(const vector<btrblocks::Range>& ranges, SIZE chunk_i) const {
	...
	// -------------------------------------------------------------------------------------
	auto c_columns = std::unique_ptr<std::unique_ptr<u8[]>[]>(new std::unique_ptr<u8[]>[columns.size()]);
	auto c_bitmaps = std::unique_ptr<std::unique_ptr<BITMAP[]>[]>(new std::unique_ptr<BITMAP[]>[columns.size()]);
	...
	for (u32 i = 0; i < columns.size(); i++) {
		...
		std::memcpy(reinterpret_cast<void*>(c_bitmaps[i].get()),
		            columns[i].bitmaps().data + offset,
		...
		case ColumnType::INTEGER: {
			...
			std::memcpy(reinterpret_cast<void*>(c_columns[i].get()),
			            columns[i].integers().data + offset,
	...
	// -------------------------------------------------------------------------------------
	return Chunk(std::move(c_columns), std::move(c_bitmaps), chunk_tuple_count, *this, std::move(c_sizes));
}
// -------------------------------------------------------------------------------------
InputChunk Relation::getInputChunk(const Range& range, [[maybe_unused]] SIZE chunk_i, u32 column) const {
	auto offset            = std::get<0>(range);
	auto chunk_tuple_count = std::get<1>(range);

	auto bitmap = std::unique_ptr<BITMAP[]>(new BITMAP[chunk_tuple_count * sizeof(BITMAP)]);
	std::memcpy(reinterpret_cast<void*>(bitmap.get()),
	            columns[column].bitmaps().data + offset,
	            chunk_tuple_count * sizeof(BITMAP));
	...
	case ColumnType::INTEGER: {
		...
		std::memcpy(reinterpret_cast<void*>(data.get()),
		            columns[column].integers().data + offset,
		            chunk_tuple_count * sizeof(INTEGER));
		break;
	}
	case ColumnType::DOUBLE: {
		...
		data = std::unique_ptr<u8[]>(new u8[size]);
		std::memcpy(reinterpret_cast<void*>(data.get()),
		            columns[column].doubles().data + offset,
		            chunk_tuple_count * sizeof(DOUBLE));
		break;
	}
	case ColumnType::STRING: {
		...
		const u64 strings_size = (source_slots[offset + chunk_tuple_count - 1].offset - source_slots[offset].offset) +
		                         source_slots[offset + chunk_tuple_count - 1].size;
		// -------------------------------------------------------------------------------------
		...
		// copy the strings
		std::memcpy(
		    reinterpret_cast<void*>(data.get() + slots_size), columns[column].strings()[offset].data(), strings_size);
		// -------------------------------------------------------------------------------------
		break;
	}
	default:
...
// -------------------------------------------------------------------------------------
void Relation::verifyTupleCounts() {
	if (columns.size()) {
...
std::shared_ptr<c3::RowGroup> Relation::getRowGroup(btrblocks::Range range) {
	...
	std::vector<std::shared_ptr<InputChunk>> column_chunks;
	for (size_t i = 0; i < columns.size(); i++) {
		column_chunks.push_back(std::make_shared<InputChunk>(std::move(getInputChunk(range, 0, i)))); // second argument unused
	}

	return std::make_shared<c3::RowGroup>(column_chunks, column_names, std::get<1>(range));
}

std::vector<std::vector<InputChunk>> Relation::getInputChunks(std::vector<btrblocks::Range> ranges){
	std::vector<std::vector<InputChunk>> inputChunks;
	for(const auto& range: ranges){
		std::vector<InputChunk> row_group_input_chunks;
		for(size_t col = 0; col < columns.size(); col++){
			row_group_input_chunks.push_back(std::move(getInputChunk(range, 0, col))); // second argument unused
		}
		inputChunks.push_back(std::move(row_group_input_chunks));
	}
	return inputChunks;	
}
